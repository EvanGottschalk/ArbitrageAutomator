...
// interfaces with the 1inch decentralized exchange
contract IOneSplit {
    // determines whether there is an arbitrage opportunity or not
    ...
    // performs one of the swaps involved in arbitrage
    function swap(
        IERC20 fromToken,
        IERC20 toToken,
        uint256 amount,
...
contract TradingBot is DyDxFlashLoan {
    ...
    modifier onlyOwner() {
        require(msg.sender == OWNER, "caller is not the owner!");_;
    }
    ...
    // entry point of arbitrage where funds are initially obtained
    ...
    // tracks the smart contract balance before and after the _trade function
    // if the final balance is less than the initial balance, execution is canceled
